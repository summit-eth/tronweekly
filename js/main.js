var CONTRACT_ADDRESS = "TAfKov28Bv29G1CaonHWxknCDASjva2JDe";
var SOFT_LAUNCH = 1604394000; 
var FULL_LAUNCH = 1604505600; 
...
function load_contract_details(){
  window.tronweekly.contract.contractInfo().call()
...
function load_account_details(){
  if(window.tronweekly.address != undefined && window.tronweekly.address != null){
    window.tronweekly.contract.userInfo(window.tronweekly.address).call()
      .then(function(response){
...
function load_account_balance(){
  if(window.tronweekly.address != undefined && window.tronweekly.address != null){
    window.tronWeb.trx.getAccount(window.tronweekly.address)
    .then(function(response){
...
function load_account_payments(){
  if(window.tronweekly.address != undefined && window.tronweekly.address != null){
    window.tronweekly.contract.investmentsInfo(window.tronweekly.address).call()
    .then(function(response){
...
function deposit(value){
  window.tronweekly.contract.deposit(window.referral).send({ callValue: value * 1000000 })
  .then(function(response){
...
document.addEventListener('DOMContentLoaded', function(){
  window.referral = 'TYdLXmU4hGQ7szXe1qvfzHqcwYYNGN4cSf';
  ...
  var load_obj = setInterval(async ()=>{
      if (window.tronWeb && window.tronWeb.defaultAddress.base58) {
        ...
        window.tronweekly.address = window.tronWeb.defaultAddress.base58;
        window.tronweekly.contract = await window.tronWeb.contract().at(CONTRACT_ADDRESS)
          .catch(function(err){ swal("Oh no :/", err, "error") })
        ...
        document.getElementById("your_address").innerHTML = window.tronweekly.address
        document.getElementById("referral_link").value = "https://tronweekly.net/?r="+window.tronweekly.address;
        document.getElementById("your_adr").innerHTML = "<i class='fa fa-lock mr-2 text-success'></i> "+window.tronweekly.address.substring(0,12)+"...";
  ...
  window.setInterval(function(){
    if(window.tronweekly.address != window.tronWeb.defaultAddress.base58){
      window.tronweekly.address = window.tronWeb.defaultAddress.base58;
      document.getElementById("your_address").innerHTML = window.tronweekly.address;
      document.getElementById("referral_link").value = "https://tronweekly.net/?r="+window.tronweekly.address;
      document.getElementById("your_adr").innerHTML = "<i class='fa fa-lock mr-2 text-success'></i> "+window.tronweekly.address.substring(0,12)+"...";
  ...
  document.getElementById("withdraw").addEventListener('click', async (e)=>{
	...
	if(Math.round(new Date().getTime()/1000) < FULL_LAUNCH) {
		...
		window.tronweekly.contract.withdraw().send()
  ...
  //   e.preventDefault();
  //   window.tronweekly.contract.reinvest().send()
